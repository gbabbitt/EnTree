...
# read through variant list
inlist = open("pdb_mutants/mutants.txt", "r")
for y in range(len(inlist_lines)):
    inlist_line = inlist_lines[y]
    #print(inlist_line)
    inlist_line_array = str.split(inlist_line, "\t")
    listID = inlist_line_array[0]
    #print(listID)

    # read MD ctl file
    infile = open("MDq_"+listID+".ctl", "r")
