...
#### Declare variables ####
my $varlist = 'pdb_mutants/mutants.txt';
my $fileIDq = '';
my $fileIDr = '1rex';
...
my $pdbFrame = $mw->Frame();
	...
	#				);
	my $varlistFrame = $pdbFrame->Frame();
		my $varlistLabel = $varlistFrame->Label(-text=>"list of variants (e.g. pdb_mutants/mutants.txt ) : ");
		my $varlistEntry = $varlistFrame->Entry(-borderwidth => 2,
					-relief => "groove",
					-textvariable=>\$varlist
					);
...
my $doneButton = $mw -> Button(-text => "create PDB files for delta delta G prediction", 
				-command => \&done
				); # Creates a file button
...
$varlistLabel->pack(-side=>"left");
$varlistEntry->pack(-side=>"left");
...
open(MUT, "<"."pdb_mutants/mutants.txt");
my @MUT = <MUT>;
#print @MUT;
for (my $p = 0; $p < scalar @MUT; $p++){
	 ...
	 $fileIDr = $MUTrow[0];
      print "\nmaking control file for $fileIDr.pdb\n";
      sleep(2);   
...
open(my $ctlFile2, '>', "MDr_$fileIDr.ctl") or die "Could not open output file";
print $ctlFile2 
...
print ctlFile4 "atomicfluct out fluct_$fileIDr"."_$i.txt \@CA,C,O,N,H&!(:WAT) byres\n";
...
if ($reduce_enter eq "y"){system "pdb4amber -i pdb_mutants/$fileIDr.pdb -o ".$fileIDr."REDUCED.pdb --dry --reduce \n";}
if ($reduce_enter eq "n"){system "pdb4amber -i pdb_mutants/$fileIDr.pdb -o ".$fileIDr."REDUCED.pdb --dry \n";}
...
if ($reduce_enter eq "yes"){system "pdb4amber -i pdb_mutants/$fileIDr.pdb -o ".$fileIDr."REDUCED.pdb --dry --reduce \n";}
if ($reduce_enter eq "no"){system "pdb4amber -i pdb_mutants/$fileIDr.pdb -o ".$fileIDr."REDUCED.pdb --dry \n";}
sleep(1);
close MUT;
} # end variant list loop
open(MUT, "<"."pdb_mutants/mutants.txt");
...
# create PDB file from equilibrated MD runs
print "creating PDB file for $fileIDr\n\n";
open(PDB, ">"."create-pdb$fileIDr.cpptraj");
print PDB "parm wat_$fileIDr"."REDUCED.prmtop\n";
print PDB "trajin prod_$fileIDr"."REDUCED_0.nc lastframe\n";
print PDB "autoimage\n";
print PDB "rms fit :1-306\n";
print PDB "strip :WAT\n";
print PDB "strip :Na+,Cl-\n";
print PDB "trajout pdb_mutants_final/final_$fileIDr.pdb pdb\n";
print PDB "run\n";
close PDB;
system "cpptraj -i create-pdb$fileIDr.cpptraj\n";

# open .out files for data
print "MD output files copied\n\n";
sleep(2);
mkdir MDoutput;
for (my $i = 0; $i < $runsID; $i++){
...
# retrieve original structural template file
#print "\nEnter the PDB ID for your original structure from which variants were created (e.g. 1rex.pdb)\n\n";
#$PDBidnum = <STDIN>;
#chop($PDBidnum);
#system "pdb4amber -i $PDBidnum -o "."pdb_mutants_final/$PDBidnum --dry --reduce \n";
