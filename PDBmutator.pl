...
my $pdbFrame = $mw->Frame();
	my $fileFrame = $pdbFrame->Frame();
		my $fileLabel = $fileFrame->Label(-text=>" reference structure - pdb ID (e.g. 1rex) : ");
...
my $closeButton = $mw -> Button(-text => "close PDBmutator", 
				-command => \&close
				); # Creates a program close button
...
copy("$fileID.pdb", "$outdir/$fileID.pdb");
}
...
print "\nENTER THE NUMBER OF VARIANTS + ANCESTORS IN YOUR .aln FILE (i.e. number of rows minus one)\n\n";
$varnum = <STDIN>;
chop($varnum);
sleep(1);
$Vnum = $varnum;
...
####################################################################
sub close{
 print "NOTE:structures will later be reduced, energy minimized, and MD equilibrated\n";	
 sleep(1);	
 print "\nclosing PDBmutator\n";
 exit;
}
