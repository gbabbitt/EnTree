...
my $pathFrame = $mw->Frame();
	...
	my $maestroFrame = $pathFrame->Frame();
		my $maestroLabel = $maestroFrame->Label(-text=>"path to maestro executable (e.g. /home/greg/Desktop/EnTree/) : ");
		my $maestroEntry = $maestroFrame->Entry(-borderwidth => 2,
					-relief => "groove",
					-textvariable=>\$maestro_path
					);
...
$maestroLabel->pack(-side=>"left");
$maestroEntry->pack(-side=>"left");
...
maestro_path\t$maestro_path\t# path to maestro executable\n";
close $ctlFile;
